...
Проєкт будується виключно на **безкоштовних технологіях**: Google Gemini 2.0 Flash (ШІ), Tavily API (пошук), Supabase (PostgreSQL БД), Vercel (деплой фронту), Render (деплой бекенду).
...
- **Бекенд (`backend/`):** FastAPI. Вже є кастомний `ReAct` цикл (ми навмисно **не використовуємо** LangChain). Модель Gemini вміє викликати інструмент пошуку Tavily.
...
3. Вставте ваші ключі від Gemini, Tavily та Supabase у файл `.env`. Без них ШІ-агент не зможе генерувати відповіді.
